...
def compute_monthly_commission(
    ...
    referrals = (
        ...
        .filter(
            AffiliateReferral.affiliate_user_id == affiliate_user_id,
            AffiliateReferral.activated_at.isnot(None),
        )
        .all()
    )
    referred_tenants: List[str] = [r.referred_tenant_id for r in referrals]
    ...
    return {
        ...
        "percent": percent,
        "amount": amount,
        "tenants": referred_tenants,
        "activated_referrals": len(referrals),
