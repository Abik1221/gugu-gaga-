...
from app.models.supplier import Supplier, Order
...
def list_suppliers_admin(
    ...
    page: int = 1,
    page_size: int = 20,
    _rl=Depends(rate_limit_user("admin_list_suppliers")),
):
    """List all suppliers for admin review"""
    page = max(1, page)
    page_size = max(1, min(100, page_size))
    
    query = db.query(Supplier).join(User, Supplier.user_id == User.id)
    if q:
        like = f"%{q}%"
        query = query.filter(
    ...
    for supplier in suppliers:
        ...
        order_count = db.query(Order).filter(Order.supplier_id == supplier.id).count()
        ...
        items.append({
            ...
            "user_approved": user.is_approved if user else False,
        })
...
def approve_supplier(
    ...
    db: Session = Depends(get_db),
    current_user=Depends(get_current_user),
    _rl=Depends(rate_limit_user("admin_approve_supplier")),
):
    ...
    log_event(
        ...
        metadata={"company_name": supplier.company_name}
    )
    ...
    create_notification(
        ...
        body="Your supplier account has been approved. You can now access your dashboard."
    )
    ...
    if user.email:
        send_email(
            ...
            "Your supplier account has been approved. You can now log in and start managing your products and orders."
        )
    
    return {"supplier_id": supplier_id, "status": "approved"}


@router.post("/suppliers/{supplier_id}/reject")
def reject_supplier(
    supplier_id: int,
    _=Depends(require_role(Role.admin)),
    db: Session = Depends(get_db),
    current_user=Depends(get_current_user),
    _rl=Depends(rate_limit_user("admin_reject_supplier")),
):
    """Reject supplier KYC"""
    supplier = db.query(Supplier).filter(Supplier.id == supplier_id).first()
    if not supplier:
    ...
    create_notification(
        ...
        body="Your supplier account application has been rejected."
    )
    ...
    if user.email:
        send_email(
            ...
            "Your supplier account application has been rejected. Please contact support for more information."
        )
    
    return {"supplier_id": supplier_id, "status": "rejected"}


@router.get("/suppliers/{supplier_id}/orders")
def get_supplier_orders_admin(
    supplier_id: int,
    _=Depends(require_role(Role.admin)),
    db: Session = Depends(get_db),
    _rl=Depends(rate_limit_user("admin_supplier_orders")),
):
...
def list_supplier_payments_admin(
    _=Depends(require_role(Role.admin)),
    db: Session = Depends(get_db),
    status_filter: str | None = None,
    _rl=Depends(rate_limit_user("admin_supplier_payments")),
):
    """List supplier payment submissions"""
    query = db.query(SupplierPaymentSubmission).join(Supplier)
    if status_filter:
        query = query.filter(SupplierPaymentSubmission.status == status_filter)
    
    payments = query.order_by(SupplierPaymentSubmission.id.desc()).all()
    
    items = []
    for payment in payments:
        supplier = db.query(Supplier).filter(Supplier.id == payment.supplier_id).first()
        user = db.query(User).filter(User.id == supplier.user_id).first() if supplier else None
        
        items.append({
            "id": payment.id,
            "supplier_id": payment.supplier_id,
            "company_name": supplier.company_name if supplier else None,
            "supplier_email": user.email if user else None,
            "code": payment.code,
...
@router.post("/suppliers/payments/{payment_id}/verify")
def verify_supplier_payment(
    payment_id: int,
    _=Depends(require_role(Role.admin)),
    db: Session = Depends(get_db),
    current_user=Depends(get_current_user),
    _rl=Depends(rate_limit_user("admin_verify_supplier_payment")),
):
    """Verify supplier payment submission"""
    payment = db.query(SupplierPaymentSubmission).filter(
        SupplierPaymentSubmission.id == payment_id
    ).first()
    ...
    log_event(
        ...
        tenant_id=None,
        actor_user_id=current_user.id,
        action="supplier_payment_verified",
        target_type="supplier_payment",
        target_id=str(payment_id),
        metadata={"code": payment.code, "supplier_id": payment.supplier_id}
    )
    ...
    if supplier:
        ...
        if user:
            create_notification(
                ...
                type="payment_verified",
                title="Payment Verified",
                body="Your payment has been verified. You can now access your supplier dashboard."
            )
            ...
            if user.email:
                send_email(
                    ...
                    "Your payment has been verified and your supplier account is now fully activated. You can log in to access your dashboard."
                )
    
    return {"payment_id": payment_id, "status": "verified"}


@router.post("/suppliers/payments/{payment_id}/reject")
def reject_supplier_payment(
    payment_id: int,
    _=Depends(require_role(Role.admin)),
    db: Session = Depends(get_db),
    current_user=Depends(get_current_user),
    _rl=Depends(rate_limit_user("admin_reject_supplier_payment")),
):
    """Reject supplier payment submission"""
    payment = db.query(SupplierPaymentSubmission).filter(
        SupplierPaymentSubmission.id == payment_id
    ).first()
    ...
    log_event(
        ...
        tenant_id=None,
        actor_user_id=current_user.id,
        action="supplier_payment_rejected",
        target_type="supplier_payment",
        target_id=str(payment_id),
        metadata={"code": payment.code, "supplier_id": payment.supplier_id}
    )
    ...
    if supplier:
        ...
        if user:
            create_notification(
                ...
                type="payment_rejected",
                title="Payment Rejected",
                body="Your payment submission has been rejected. Please submit a valid payment code."
            )
            ...
            if user.email:
                send_email(
                    ...
                    "Your payment submission has been rejected. Please verify your payment details and submit a valid payment code."
                )
