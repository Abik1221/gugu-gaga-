...
def register_pharmacy(payload: PharmacyRegister, db: Session = Depends(get_db)):
    ...
    return UserOut(id=owner.id, email=owner.email, phone=owner.phone, role=owner.role)
...
def register_affiliate(payload: AffiliateRegister, db: Session = Depends(get_db)):
    ...
    return UserOut(id=user.id, email=user.email, phone=user.phone, role=user.role)
...
from app.schemas.auth import Token, UserCreate, UserLogin, UserOut, PharmacyRegister, AffiliateRegister
...
def register(payload: UserCreate, db: Session = Depends(get_db), tenant_id: Optional[str] = Depends(get_optional_tenant_id)):
    ...
    # Direct affiliates and pharmacy owners to the dedicated registration endpoints
    if role_value == Role.affiliate.value:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Use /auth/register/affiliate for affiliate registration",
        )
    if role_value == Role.pharmacy_owner.value:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Use /auth/register/pharmacy for pharmacy registration",
        )
